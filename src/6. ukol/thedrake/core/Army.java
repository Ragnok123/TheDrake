...
import thedrake.core.board.BoardTroops;
...
public class Army implements JSONSerializable {
	...
	public void toJSON(PrintWriter writer) {
		...
		writer.print(",\"stack\":[");
		Iterator<Troop> stacks = stack.iterator();
		while (stacks.hasNext()) {
			writer.print("\"" + stacks.next().name() + "\"");
			if (stacks.hasNext())
				writer.print(",");
		}
		writer.print("]");
		writer.print(",\"captured\":[");
		Iterator<Troop> capitulated = captured.iterator();
		while (capitulated.hasNext()) {
			writer.print("\"" + capitulated.next().name() + "\"");
			if (capitulated.hasNext())
				writer.print(",");
		}
		writer.print("]}");
